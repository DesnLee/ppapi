...
func (ctl *ItemController) Register(g *gin.RouterGroup) {
	...
	v1.GET("/items/balance", ctl.GetBalance)
}
...
func (ctl *ItemController) GetBalance(c *gin.Context) {
	...
	if err := controller_helper.ValidateGetBalanceRequestBody(&body); err != nil {
		c.JSON(http.StatusUnprocessableEntity, model.MsgResponse{
			Msg: err.Error(),
		})
		return
	...
	res, err := controller_helper.GetBalance(userID, body)
	if err != nil {
		c.JSON(http.StatusInternalServerError, model.MsgResponse{
			Msg: err.Error(),
		})
